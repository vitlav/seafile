...
parse_head_commit_info (const char *rsp_content, int rsp_size, CheckHeadData *data)
{
    ...
    object = json_loadb (rsp.content, rsp.size, 0, &jerror);
    if (!object) {
...
check_head_commit_thread (void *vdata)
{
    ...
    url = g_strdup_printf ("%s/sync/repo/%s/commit/HEAD", data->host, data->repo_id);

    token_header = g_strdup_printf ("Seafile-Repo-Token: %s", data->token);
    headers = curl_slist_appned (headers, token_header);
    g_free (token_header);
